...
Use a hosted Postgres URL in Vercel for `DATABASE_URL` (and ideally `DIRECT_DATABASE_URL`), for example:

```env
DATABASE_URL="postgresql://user:password@db-host:5432/nrb_europe?sslmode=require"
DIRECT_DATABASE_URL="postgresql://user:password@db-host:5432/nrb_europe?sslmode=require"
```

Do not use local values like `file:./prod.db` or `localhost` in Vercel.
