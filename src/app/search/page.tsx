    ...
    <div className="nrb-container py-8">
      <div className="mb-8">
        <h1 className="headline-2xl mb-2 flex items-center gap-2">
          <Search className="w-8 h-8 text-nrb-red" />
          {query ? `Search Results for "${query}"` : 'Search News'}
        </h1>
        <p className="text-nrb-gray">
          {query ? `Found ${articles.length} result${articles.length !== 1 ? 's' : ''}` : 'Enter search terms above'}
        </p>
      </div>

      {query.length < 2 ? (
        <div className="nrb-card text-center py-20">
          <Search className="w-16 h-16 text-nrb-gray mx-auto mb-4" />
          <p className="text-nrb-text text-lg">Please enter at least 2 characters to search.</p>
        </div>
      ) : articles.length === 0 ? (
        <div className="nrb-card text-center py-20">
          <Search className="w-16 h-16 text-nrb-gray mx-auto mb-4" />
          <p className="text-nrb-text text-lg mb-2">No articles found for "{query}"</p>
          <p className="text-sm text-nrb-gray">
