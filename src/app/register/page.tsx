...
function RegisterForm() {
  ...
  const postRegisterWithRetry = async (payload: {
    name: string
    email: string
    password: string
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await Sentry.startSpan(
        ...
        async () => {
          return await postRegisterWithRetry({
            name: trimmedName,
            email: sanitizedEmail,
            password,
            isAdminRegistration: isAdminRegister,
            adminSetupCode: isAdminRegister ? adminSetupCode : undefined,
          })
        },
      )
      ...
      if (!res.ok) {
        ...
        setTimeout(() => {
          if (isAdminRegister) {
            const next = safeCallbackUrl ? `&callbackUrl=${encodeURIComponent(safeCallbackUrl)}` : ''
            router.push(`/login?admin=true&registered=true${next}`)
          } else {
      ...
      <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md w-96">
        ...
        {isAdminRegister && (
          <div className="bg-amber-100 border border-amber-400 text-amber-800 px-4 py-3 rounded mb-4 text-sm">
            Admin account creation requires the owner setup code.
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} autoComplete="on">
          ...
          {isAdminRegister && (
            <div className="mb-6">
              <label htmlFor="adminSetupCode" className="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">
                Owner Setup Code
              </label>
              <input
                id="adminSetupCode"
          ...
          >
            {loading ? 'Creating Account...' : (isAdminRegister ? 'Create Admin Account' : 'Create Account')}
          </button>
        </form>
        ...
        <p className="text-center text-sm text-gray-600 dark:text-gray-400 mt-4">
          ...
          <Link href={isAdminRegister ? '/login?admin=true&callbackUrl=/admin' : '/login'} className="text-blue-600 hover:underline font-semibold">
            Sign In
          </Link>
        </p>
